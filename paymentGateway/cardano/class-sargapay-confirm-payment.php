    ...
    function sargapay_check_all_pendding_orders()
    {
        ...
        if ($wpdb->last_error === "" && count($orders) !== 0) {
            for ($i = 0; $i < count($orders); $i++) {
                ...
                // hours from settings in seconds            
                $time_wait_settings = isset(WC()->payment_gateways->payment_gateways()['sargapay_cardano']->time_wait) ?
                    WC()->payment_gateways->payment_gateways()['sargapay_cardano']->time_wait : 24;
                $time_wait =  intval($time_wait_settings) * 60 * 60;
                ...
                if ($confirmation_obj->error === 0) {
                    ...
                    if ($confirmation_obj->confirmations > 0) {
                        if ($confirmation_obj->confirmations > WC()->payment_gateways->payment_gateways()['sargapay_cardano']->confirmations) {
